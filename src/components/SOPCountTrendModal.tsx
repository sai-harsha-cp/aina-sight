...
interface SOPCountTrendModalProps {
  isOpen: boolean;
  onClose: () => void;
  sopName: string;
  currentValue: number;
  unit: string; // e.g., "sessions", "alerts", "SKUs", ""
}
...
// Generate mock trend data for count-based metrics
const generateTrendData = (timeline: Timeline, currentValue: number) => {
  let dataPoints: { date: string; value: number }[] = [];
  const variation = Math.max(3, Math.round(currentValue * 0.15)); // 15% variation
  ...
  switch (timeline) {
    case '7days':
      for (let i = 6; i >= 0; i--) {
        ...
        const value = Math.max(0, currentValue + variance - i);
        dataPoints.push({
    ...
    case '30days':
      for (let i = 29; i >= 0; i--) {
        ...
        date.setDate(date.getDate() - i);
        const variance = Math.round((Math.random() - 0.5) * variation * 2);
        const value = Math.max(0, currentValue + variance - Math.floor(i * 0.3));
        dataPoints.push({
    ...
    case '90days':
      for (let i = 89; i >= 0; i--) {
        ...
        date.setDate(date.getDate() - i);
        const variance = Math.round((Math.random() - 0.5) * variation * 2);
        const value = Math.max(0, currentValue + variance - Math.floor(i * 0.1));
        dataPoints.push({
    ...
    case 'all':
      for (let i = 11; i >= 0; i--) {
        ...
        const variance = Math.round((Math.random() - 0.5) * variation * 2);
        const value = Math.max(0, currentValue + variance - Math.floor(i * 0.8));
        dataPoints.push({
...
export function SOPCountTrendModal({ isOpen, onClose, sopName, currentValue, unit }: SOPCountTrendModalProps) {
      ...
      <DialogContent className="max-w-7xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="grid grid-cols-3 gap-4 mt-6">
          <div className="p-4 bg-gray-50 rounded-lg">
            ...
            <p className="text-gray-900">{currentValue}{unit && ` ${unit}`}</p>
          </div>
          <div className="p-4 bg-gray-50 rounded-lg">
            <p className="text-gray-600 text-sm mb-1">Average</p>
            <p className="text-gray-900">{avgValue}{unit && ` ${unit}`}</p>
          </div>
          <div className="p-4 bg-gray-50 rounded-lg">
            <p className="text-gray-600 text-sm mb-1">Range</p>
            <p className="text-gray-900">{minValue} - {maxValue}{unit && ` ${unit}`}</p>
          </div>
        </div>
          ...
          <ResponsiveContainer width="100%" height={350}>
            <LineChart data={trendData} margin={{ top: 10, right: 30, left: 0, bottom: 10 }}>
              ...
              <Tooltip
                ...
                formatter={(value: number) => [unit ? `${value} ${unit}` : value, 'Count']}
              />
              <Line 
                type="monotone" 
                dataKey="value" 
